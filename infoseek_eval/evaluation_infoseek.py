...
def evaluate_infoseek_full(
    ...
    infoseek_score = []
    for pred, qid2example in tqdm(zip(predictions, qid2examples), total=len(predictions), desc="Infoseek Evaluation"):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--reference_path', type=str, default=None, help="Path to reference jsonl")
    parser.add_argument('--reference_qtype_path', type=str, default=None, help="Path to reference qtype jsonl")
    parser.add_argument('--split', type=str, choices=['val', 'test'], default='val')
    args = parser.parse_args()

    reference_path = args.reference_path if args.reference_path else f"path_to_infoseek_{args.split}.jsonl"
    reference_qtype_path = args.reference_qtype_path if args.reference_qtype_path else f"path_to_infoseek_{args.split}_qtype.jsonl"

    for i, pred_path in enumerate(args.input_path):
        print(f"=== {pred_path.split('/')[-1]} ===")
        
        merge_data = f'{pred_path}/merge.jsonl'
        files = glob(os.path.join(pred_path, 'split_*.json'))
        data = []
        for z in files:
            try:
                with open(z, 'r') as f:
        ...
        result = evaluate(merge_data, reference_path, reference_qtype_path, args.adjust_score)
