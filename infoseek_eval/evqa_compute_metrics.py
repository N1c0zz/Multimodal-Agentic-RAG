...
# wget https://storage.googleapis.com/cloud-tpu-checkpoints/bert/keras_bert/uncased_L-12_H-768_A-12/vocab.txt
_VOCAB_PATH = '/work/cvcs2026/feature_extractors/dati_progetto/vocab.txt' #'gs://cloud-tpu-checkpoints/bert/keras_bert/uncased_L-12_H-768_A-12/vocab.txt'
# TFHUB_CACHE_DIR= python -c "import tensorflow_hub as hub; hub.load('https://tfhub.dev/google/answer_equivalence/bem/1')"
_MODEL_PATH = 'https://tfhub.dev/google/answer_equivalence/bem/1' # 'https://tfhub.dev/google/answer_equivalence/bem/1'
...
def initialize_bem_scoring_function(vocab_path=_VOCAB_PATH,
  ...
  def pad(a, length=512):
      if a.shape[-1] > length:
          a = a[:length]  # taglia i token in eccesso
...
def main():
  ...
  for p in args.input_path:
    ...
    if '.json' in p:
      ...
      files = glob.glob(os.path.join(p, 'split_*.json'))
      files = [f for f in files if 'lat' not in f]
      #files = [os.path.join(p, f) for f in listdir(p) if isfile(join(p, f)) and ('results' not in f)]
      data = []
      for z in files:    
        with open(z, 'r') as f:
      ...
      try:
        with open(os.path.join(p, 'merge.json'), 'w') as f:
          ujson.dump(data, f)
      except:
        pass
      
    for eval_type in ['all', 'w_evidence', 'wo_evidence']:
      single_hop_scores = []
      two_hop_scores = []
      all_scores = []
      for element in tqdm(data, mininterval=1, desc=f'Evaluating {eval_type}'):
        if eval_type == 'w_evidence' and 'evidence_in_context' in element and not element['evidence_in_context']:
          continue
        if eval_type == 'wo_evidence' and 'evidence_in_context' in element and element['evidence_in_context']:
          continue
        reference_list = element['reference']
        if isinstance(reference_list, str):
          reference_list = reference_list.split('|')
        score = evaluate_example(question=element['question'], reference_list=reference_list, candidate=element['answers'][:128], question_type=element['question_type'])
        if element['question_type'] != '2_hop':
          single_hop_scores.append(score)
        if element['question_type'] == '2_hop':
          two_hop_scores.append(score)
        all_scores.append(score)
      
      print(f'Number of examples with single hop = {len(single_hop_scores)}')
      print(f'Number of total examples = {len(all_scores)}')
      single_score = sum(single_hop_scores) / len(single_hop_scores) if single_hop_scores else 0.0
      all_score = sum(all_scores) / len(all_scores) if all_scores else 0.0
      two_score = sum(two_hop_scores) / len(two_hop_scores) if two_hop_scores else 0.0
      print(f'Score Single-Hop = {single_score}')
      print(f'Score Two-Hop = {two_score}')
      print(f'Score All = {all_score}')
      print(f'Process finished...')
      
      meta_result = {}
      meta_result['Number_single_hop'] = len(single_hop_scores)
      meta_result['Number_total'] = len(all_scores)
      meta_result['single_hop_score'] = single_score
      meta_result['two_hop_score'] = two_score
      meta_result['all_score'] = all_score
      meta_result['experiment'] = p
      with open(f'{p}/results{eval_type if eval_type != "all" else ""}.json', 'w') as f:
