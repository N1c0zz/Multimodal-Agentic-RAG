...
- **Passage-Level Critic Filtering:** Overcomes the classic "context truncation vs. noise" trade-off. The system retrieves *full documents* and employs a high-resolution **ReAG-Critic** model to filter out irrelevant passages individually, feeding the agent a highly condensed context.
- **Graceful Degradation:** A code-level fallback mechanism intercepts situations where the Critic filters out all retrieved evidence or the agent outputs "bailout" patterns (e.g., "Unknown"). This forces the system to rely on pure zero-shot visual reasoning rather than hallucinating over empty or noisy contexts.
...
<p align="center">
  <img src="doc/final_architecture.png" alt="Architecture Diagram" width="85%">
</p>

1. **`assess_retrieval_need`**: The agent visually inspects the image and determines if external knowledge is required.
2. **`retrieve_knowledge`**: Executes the Entity Guessing fusion, fetches the top-$k$ full documents from FAISS, and passes them to the ReAG-Critic for fine-grained section filtering.
